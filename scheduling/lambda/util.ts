...
import { SchedulingResult } from './provider';
...
export const modifySchedulingResultSuccess = (SchedulingResult: SchedulingResult) => {
    //explicity converting it to any, to avoid type check when converting scheduling success boolean to number
    const result: any = JSON.parse(JSON.stringify(SchedulingResult));
    result.schedulingSuccess = SchedulingResult.schedulingSuccess ? 1 : 0;
    return result;
}