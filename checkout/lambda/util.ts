...
import { CheckoutResult } from './checkout-provider';
...
// util to convert from boolean to number
export const modifyCheckoutResultErrorBoolean= (checkoutResult: CheckoutResult) => {
    //explicity converting it to any, to avoid type check when converting  boolean to number
    let modifiedResult: any = JSON.parse(JSON.stringify(checkoutResult));
    modifiedResult.error = modifiedResult.error ? 1 : 0;
